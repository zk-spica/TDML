...
int main(int argc, char ** argv)
{
	...
	FILE *fpin1=fopen(argv[1], "r"), *fpin2=fopen("Type__", "r"), *fpin3=fopen("head", "r"), *fpin4=fopen("tail", "r"), *fpout=fopen(argv[2], "w"), *fptmp=fopen("__tmp", "w");
	char c;
	while (fscanf(fpin3, "%c", &c)!=EOF)
		fprintf(fpout, "%c", c);
	...
	fprintf(fpout, "\n};\n");
	while (fscanf(fpin4, "%c", &c)!=EOF)
		fprintf(fpout, "%c", c);
	fclose(fpout);
	//system( (string("g++ -std=c++11 -o __run ")+string(argv[2])).c_str() );
	//system("__run");
	return 0;
}
